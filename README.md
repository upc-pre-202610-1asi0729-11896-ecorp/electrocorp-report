...
|**US-29**|Recibir alertas en la aplicación|<p>Como usuario, quiero que el sistema me notifique dentro de la aplicación si algo inusual pasa con mi consumo.</p>|<p>**Escenario 1:** Alerta in-app</p><p>Dado que hay un evento crítico registrado,</p><p>Cuando el backend evalúa las reglas de alerta,</p><p>Entonces la alerta aparece en el centro de notificaciones del usuario.</p>|**EP-07**|
|**US-30**|Consultar reporte mensual de consumo|<p>Como usuario, quiero consultar un resumen mensual de mi consumo para saber cuánto gasté y ahorré.</p>|<p>**Escenario 1:** Reporte disponible</p><p>Dado que finaliza el periodo mensual,</p><p>Cuando el usuario abre la sección de reportes,</p><p>Entonces visualiza el resumen de consumo del mes.</p>|**EP-07**|
...
|**TS-02**|Internacionalización client-side del Landing|<p>**Como** desarrollador frontend</p><p>**Quiero** implementar el cambio de idioma en el navegador con diccionarios de traducción</p><p>**Para** alternar dinámicamente entre ES/EN sin recargar la página (US-03).</p>|<p>**Escenario 1: Traducción dinámica**</p><p>Dado que el Landing está cargado,</p><p>Cuando el usuario activa el selector de idioma (i18n.js),</p><p>Entonces los textos se reemplazan en el DOM al idioma seleccionado.</p>|**EP-05**|
|**TS-03**|Búsqueda client-side de secciones|<p>**Como** desarrollador frontend</p><p>**Quiero** implementar una búsqueda por palabras clave sobre las secciones del Landing</p><p>**Para** desplazar la vista a la sección correspondiente (US-02).</p>|<p>**Escenario 1: Búsqueda y scroll**</p><p>Dado que el usuario escribe un término en la barra de búsqueda,</p><p>Cuando coincide con una sección indexada (search.js),</p><p>Entonces la página hace scroll suave hacia esa sección.</p>|**EP-05**|
|**TS-04**|Formulario de newsletter client-side|<p>**Como** desarrollador frontend</p><p>**Quiero** procesar el formulario de suscripción en el navegador</p><p>**Para** registrar el interés del visitante sin recargar la página (US-04).</p>|<p>**Escenario 1: Envío del formulario**</p><p>Dado que el visitante ingresa su email,</p><p>Cuando envía el formulario (newsletter.js),</p><p>Entonces se muestra un mensaje de confirmación y se limpia el campo.</p>|**EP-05**|
|**TS-05**|Control multimedia client-side|<p>**Como** desarrollador frontend</p><p>**Quiero** gestionar el estado del menú y del audio de fondo en el navegador</p><p>**Para** sincronizar la reproducción/pausa y la navegación del Landing (US-05).</p>|<p>**Escenario 1: Control de audio y menú**</p><p>Dado que el usuario interactúa con el navbar o el botón de audio,</p><p>Cuando presiona reproducir/pausar (audio.js),</p><p>Entonces el estado del audio y del menú se actualiza en el cliente.</p>|**EP-05**|
...
|**TS-08**|Servicio de validación de pagos|<p>**Como** backend developer</p><p>**Quiero** implementar un servicio de dominio de validación de pagos</p><p>**Para** procesar el checkout de suscripciones de forma segura sin depender de una pasarela externa (US-27).</p>|<p>**Escenario 1: Checkout válido**</p><p>Dado un payload de checkout con datos de tarjeta válidos,</p><p>Cuando invoca el endpoint /subscriptions/checkout,</p><p>Entonces el servicio valida el pago y retorna la suscripción activa (HTTP 200).</p>|EP-06|
|**TS-09**|Servicio de evaluación de alertas|<p>**Como** backend developer</p><p>**Quiero** implementar un servicio que evalúe reglas de consumo</p><p>**Para** generar alertas in-app cuando se superan los umbrales (US-29).</p>|<p>**Escenario 1: Generación de alerta**</p><p>Dado que un consumo supera el umbral de una regla activa,</p><p>Cuando se ejecuta la evaluación de reglas,</p><p>Entonces se registra una alerta consultable vía /alerts.</p>|**EP-07**|
...
|**TS-17**|Integración OpenStreetMap (geolocalización)|<p>**Como** frontend developer</p><p>**Quiero** consumir los servicios externos de OpenStreetMap/Nominatim</p><p>**Para** geocodificar direcciones y mostrar un mapa interactivo al registrar sedes (US-40).</p>|<p>**Escenario 1: Geocodificación**</p><p>Dado que el usuario escribe una dirección,</p><p>Cuando la Web Application consulta el endpoint /search de Nominatim,</p><p>Entonces obtiene las coordenadas y centra el mapa en la ubicación.</p>|**EP-04**|
